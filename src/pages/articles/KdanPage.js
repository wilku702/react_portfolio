  ...
  return (
    <div className="w-full px-4 md:px-6 lg:px-10 max-w-6xl mx-auto pt-24 pb-12 space-y-12">
      <div className="space-y-6">
        <h1 className="text-4xl md:text-5xl font-bold">{kdanData.title}</h1>
        <div className="grid md:grid-cols-3 gap-6">
          <div>
            <p className="uppercase text-sm text-white/70 tracking-wide">
              service
            </p>
            <h6 className="text-lg font-monoSemi">{kdanData.service}</h6>
          </div>
          <div>
            <p className="uppercase text-sm text-white/70 tracking-wide">
              timeline
            </p>
            <h6 className="text-lg font-monoSemi">{kdanData.timeline}</h6>
          </div>
          <div>
            <p className="uppercase text-sm text-white/70 tracking-wide">
              role
            </p>
            <h6 className="text-lg font-monoSemi">{kdanData.role}</h6>
          </div>
        </div>
        <p className="text-base leading-relaxed text-white/90">
          {kdanData.description}
        </p>
      </div>
